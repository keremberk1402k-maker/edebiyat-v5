...
export default function Game() {
  ...
  const [lastAns,   setLastAns]   = useState<{idx:number|null;correct:boolean|null}>({idx:null,correct:null});
  ...
  const handleMove = (correct:boolean) => {
    ...
    const dmg=botMatch&&turn==="p2"?Math.floor(pStats.atk*0.8):pStats.atk;
    ...
    if(correct) {
      ...
      if(turn==="p1"){const np={...player!};np.hp-=25;setPlayer(np);nb.log="HATALI! -25 Can";nb.combo=0;if(np.hp<=0){np.hp=pStats.maxHp;save(np);notify("YENİLDİN...");setBattle({active:false,enemyHp:0,maxEnemyHp:0,qs:[],qIdx:0,timer:20,combo:0,log:null,wait:false,dmgText:null,shaking:false});setScreen("menu");return;}}
      else nb.log="BOT ISKALADI!";
    }
    nb.qIdx=(nb.qIdx+1)%nb.qs.length;
    setBattle(nb); setLastAns({idx:answeredIdx,correct});
    setTimeout(()=>setLastAns({idx:null,correct:null}),900);
    setTurn(p=>p==="p1"?"p2":"p1"); setBattle(nb);
  };
  ...
  // Bot savaş tetikleyicisi
  useEffect(()=>{
    if(battle.active&&botMatch&&turn==="p2"&&!battle.wait){
      const t=setTimeout(()=>{handleMove(Math.random()>0.4);},2000);
  ...
  const globalStyles=`
    ...
    @keyframes float{0%{transform:translateX(-50%) translateY(0);opacity:1}100%{transform:translateX(-50%) translateY(-50px);opacity:0}}
    @keyframes spin{to{transform:rotate(360deg)}}
    .ans-btn.correct{background:linear-gradient(90deg,#0f6,#00eaff)!important;transform:translateY(-4px)}
    .ans-btn.wrong{background:linear-gradient(90deg,#ff6b6b,#ff4b2b)!important;transform:translateY(-4px)}
    .floating-dmg{position:absolute;left:50%;font-size:72px;font-weight:800;text-shadow:0 0 30px black;animation:float 1s forwards}
  `;
        ...
        <div style={{flex:1,display:"flex",flexDirection:"column",background:`linear-gradient(rgba(0,0,0,0.6),rgba(0,0,0,0.9)),url(${battle.region?.bg||""}) center/cover`}}>
          <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"space-around",position:"relative"}}>
            ...
            <div style={{textAlign:"center",minWidth:"200px"}}>
              {battle.log&&<div style={{marginBottom:"16px",fontSize:"18px",fontWeight:"700",color:"#fc0"}}>{battle.log}</div>}
              {pvp.matchId ? (
                <div>
              ...
              <>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"14px"}}>
                  {battle.qs[battle.qIdx]?.o.map((o,i)=>{
                    const answered=lastAns.idx===battle.qIdx;
                    const cls=answered?(lastAns.correct?"ans-btn correct":"ans-btn wrong"):"ans-btn";
                    const dis=(botMatch&&turn!=="p1")||(answered&&lastAns.idx===battle.qIdx);
                    return(
                      <button key={i} className={cls} style={{...S.btn,padding:"14px",fontSize:15,width:"100%",textTransform:"none"}}
                        onClick={()=>handleMove(i===battle.qs[battle.qIdx].a)} disabled={dis}>{o}</button>
                    );
                  })}
                </div>
